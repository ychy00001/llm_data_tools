...
from common.filter import is_dirty_medical
from common.filter import preprocess_title_content
from common.utils import get_all_files

os.makedirs("./log", exist_ok=True)
log = Logger('./log/medical_dirty_clean_multi.log', level='info')
...
def run(job):
    ...
    with open(file_item_format, "a", encoding='utf-8') as f, open(
        ...
        for item_line in open(file_item, 'r', encoding='utf8'):
            ...
            text = line_json["text"]
            meta = line_json["meta"]
            dirty_info = is_dirty_medical(meta["title"], text)
            if dirty_info:
...
if __name__ == '__main__':
    # nohup python medical_dirty_clean_multi.py > log/medical_dirty_muti.out 2>&1 &
    pre_file = "medical_format_clean_250_clean"
    # 无最后斜杠
